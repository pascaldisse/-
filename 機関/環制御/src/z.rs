...
pub fn 家snap(θ: f64, 家数: u32) -> f64 {
    // D2是正 (敵対審査乙.3, 08-11): 旧実装は (-π,π] 符号付域で `.round()` していたが,
    // round()は半数を零から遠ざかる方向へ丸めるため, 正の境界は上位家へ, 負の境界は
    // 下位家へと符号でtie-break方向が反転し, 202.5° (負域に折り返される境界) のみ下位家へ
    // 倒れる非一貫が実測された (tests/敌対_丙_検算.rs). 修正: 常に正の[0,2π)域で
    // floor((θ+半刻)/刻) を取ってから環正規化する — 符号に依らず常に「境界ちょうどは上位家」
    // 規約を守る (polar.rsの度数域実装と同型).
    if 家数 == 0 {
        return 環正規化(θ);
    }
    let 刻 = 全環 / (家数 as f64);
    let θ正 = θ.rem_euclid(全環);
    let idx = ((θ正 + 刻 / 2.0) / 刻).floor();
    環正規化(idx * 刻)
}
