...
// ---------- A11 (Pascal裁定 08-11, docs/adversary 甲.2.8): 契約対象=記録済(量子化)log ----------
// 生値は非契約 — 生値と量子化logが異なる事自体は欠陥ではない (量子化=意図的な精度低下が仕様).
// 旧名 "a11_log量子化再生は生値z列とbit一致しない" は「量子化=違反」であるかのように読めた為改名し,
// 主張も「非契約の実測記録」へ是正する (assert内容は温存 — 量子化が実在する事自体は変わらぬ真).
#[test]
fn a11_生値は非契約_量子化差は仕様であり欠陥ではない() {
    let 生x = 0.123456789_f64;
    let 生y = 0.987654321_f64;
    // 梯1 main.rs / 梯2 z主.rs は wa::正準表記::表記() で量子化してlogへ書く (既定桁4 = "{:.4}"と後方互換).
    use wa::正準表記::{量子化, 正準param};
    let param = 正準param::default();
    let ログx = 量子化(生x, param);
    let ログy = 量子化(生y, param);
    assert_ne!(生x.to_bits(), ログx.to_bits(), "量子化前提確認: 生値と量子化値が既に異なるbit");
    let mut a = Z変換器::既定();
    let mut b = Z変換器::既定();
    let za = a.変換(生x, 生y);
    let zb = b.変換(ログx, ログy);
    assert_ne!(
        za.theta, zb.theta,
        "生値z列と量子化log由来z列がtheta bit一致してしまった (量子化幅次第では起こり得るが本例は不一致が期待値. \
         かつ — この不一致はA11契約違反ではない: 契約は量子化log同士のbit一致にのみ課される. 下記a13参照)"
    );
}
...
// ---------- A13 f32/f64型境界 (是正前は実機路のみ丸め経由・再生路は非経由=型経路非対称) ----------
#[test]
fn a13_是正前提の記録_f32往復丸めは実機路のみに存在し再生路には無い() {
    // 是正前の危険地帯だった事の記録 (非回帰用) — z主.rs 実機路: gilrs f32値 → `as f64`.
    // 入力源.rs 再生路: logテキスト(4桁) → parse::<f64> (f32を一切経由しない).
    let 表示相当値 = 0.1234_f64;
    let 実機路相当 = (表示相当値 as f32) as f64; // f32往復
    assert_ne!(
        表示相当値, 実機路相当,
        "f32往復で丸め誤差が発生する事の確認 (是正無しなら実機路のみ通る変換, 再生路は通らない → 型経路非対称)"
    );
}

// A13是正の実証 (Pascal裁定 08-11): 正準量子化器 (wa::正準表記) を実機路・再生路が共有すれば,
// 型経路 (f32起源 vs テキスト起源) が異なっていても bit一致する.
#[test]
fn a13_正準量子化器を経由すれば実機路と再生路はbit一致する() {
    use wa::正準表記::{表記, 量子化, 正準param};
    let param = 正準param::default();

    // 実機路模擬: device f32生値 → f64 → 量子化 (z主.rs 実機分岐が新設された箇所と同一関数).
    let 生x_f32: f32 = 0.123456789;
    let 生y_f32: f32 = 0.987654321;
    let 実機路x = 量子化(生x_f32 as f64, param);
    let 実機路y = 量子化(生y_f32 as f64, param);

    // 同じ生値を正準表記でTICK行へ書き (main.rs書出と同一関数), 入力源::log解析で読み戻す (再生路).
    let log行 = format!(
        "TICK ts=1 id=X L(x={} y={} angle=0.00 mag=0.0000 house=無) R(x=0.0000 y=0.0000 angle=0.00 mag=0.0000 house=無) L2=0.0000 R2=0.0000",
        表記(生x_f32 as f64, param),
        表記(生y_f32 as f64, param)
    );
    let 標本 = wa::入力源::log解析(&log行);
    assert_eq!(標本.len(), 1, "TICK行が解析されなかった");
    let 再生路x = 標本[0].x;
    let 再生路y = 標本[0].y;

    assert_eq!(実機路x.to_bits(), 再生路x.to_bits(), "x bit不一致 実機路={実機路x} 再生路={再生路x}");
    assert_eq!(実機路y.to_bits(), 再生路y.to_bits(), "y bit不一致 実機路={実機路y} 再生路={再生路y}");

    // 契約の本体 (A11): 量子化後の値をZ変換器へ通せば, 実機路由来・再生路由来のz列もbit一致する
    // — 「記録済logが真値源」の実証 (実機路も量子化を経由する前提, z主.rs是正済).
    let mut a = Z変換器::既定();
    let mut b = Z変換器::既定();
    let za = a.変換(実機路x, 実機路y);
    let zb = b.変換(再生路x, 再生路y);
    assert_eq!(za, zb, "量子化契約下のz列がbit一致しない (za={za:?} zb={zb:?})");
}
